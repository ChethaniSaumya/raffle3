...
async function main() {
  ...
  app.get(
    ...
    app.get('/profile', async function (req, res) {
        if (req.isAuthenticated()) {
            ...
            for(let i=0; i<tickets.length; i++){
                let event = await Event.findOne({_id: Number(tickets[i].eventId)});
                console.log(event);
                events.push(event);
            }
            console.log(tickets);
            console.log(events);
            res.render('Profile', { user: req.user , tickets: tickets  , events: events, auth: req.isAuthenticated()});
        } else {
            res.redirect("/login")
        }
    });
    ...
    app.get('/adminProfile', async function(req, res){
        if(req.isAuthenticated()){
    ...
    app.get('/endEvent/:eventId', async function (req, res) {
        if (req.isAuthenticated()) {
            ...
            const winnerTicketId = Number(event.ticketId[fixedW]);
            const winningTicket = await Ticket.findOne({_id : winnerTicketId});
            console.log({winningTicket});
            await Event.updateOne({ _id: Number(req.params.eventId) }, { eventEnd: true, winnerId: winnerId });
            const winner = await User.findOne({ _id: Number(winnerId) });
            winner.accountBalance = Number(winner.accountBalance) + Number(winningTicket.value);
            await winner.save();
            ...
            for (let i = 0; i < outputArray.length; i++) {
                if (Number(outputArray[i]) === winnerId) {
                    const notification = new Notification({
                        ...
                        notification: `Event ${event.eventName} has ended. You have won ${winningTicket.value} balance!`
                    });
                    await notification.save();
                    const customerEmail = winner.email;
                    const emailSubject = `Winner of ${event.eventName}`;
                    const emailText = `Dear ${winner.name},\nYou have won the event and have been awarded a balance of ${winningTicket.value}\n\nWith regards,\nGreat Escape team`;
                    sendOrderConfirmationEmail(customerEmail, emailText, emailSubject);
                } else {
                    const notification = new Notification({
    ...
    app.post('/processPayment', async function (req, res) {
            ...
            for (let i = 0; i < tickets.length; i++) {
                ...
                for (let j = 0; j < l; j++) {
                    ...
                    event.ticketId.push(Number(tickets[i]._id));
                }
                await event.save();
    ...
    app.post('/adminLogin', async function (req, res) {
        const temp = await User.findOne({username: req.body.username, isAdmin: true});
        const user = new User({
            username: req.body.username,
            password: req.body.password
        });
    ...
    app.post('/addEvent', async function (req, res) {
        ...
        await event.save();
        await Id.updateOne({ name: 'ids' }, { eventId: Number(ids.eventId) + 3 });
